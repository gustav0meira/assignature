        			...
        			<div style="margin-top: 20px;" class="row task">
						...
						<div class="col-2">
							<div class="row align">
								<div class="col-12">
									<div class="align">
										<form style="padding-bottom: 0px !important;" id="updateDate<?php echo $dado['id'] ?>" method="POST" action="updateDate.php">
											<input type="hidden" name="id" value="<?php echo $dado['id'] ?>">
											<input onclick="updateDate()" id="prazo<?php echo $dado['id'] ?>" style="border-color: #FFFFFF09 !important; margin-bottom: 0px;" <?php if($user['username'] == 'lancelot' OR $user['username'] == 'arthur'){}else{echo 'disabled';} ?> class="prazoTasks" type="date" value="<?php echo $dado['prazo'] ?>" name="prazo">
										</form>
										<script>
										function updateDate() {
										  document.getElementById('prazo<?php echo $dado['id'] ?>').addEventListener('change', function() {
										    document.getElementById('updateDate<?php echo $dado['id'] ?>').submit();
										  });
										  document.getElementById('prazo<?php echo $dado['id'] ?>').click();
										}
										</script>
									</div>
								</div>
